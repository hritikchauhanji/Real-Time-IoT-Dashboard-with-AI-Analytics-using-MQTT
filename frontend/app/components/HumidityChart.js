...
export default function HumidityChart({ data, liveData }) {
  ...
  useEffect(() => {
    if (liveData && liveData.length > 0) {
      ...
      setChartData((prevData) => {
        ...
        // Add all device humidity values
        liveData.forEach((device) => {
          newPoint[device.device_id] = device.humidity;
        });
  ...
  const formatDataByTimestamp = (rawData) => {
    ...
    rawData.forEach((item) => {
      ...
      timeMap.get(timestamp)[item.device_id] = item.humidity;
    });
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-bold text-gray-800">💧 Humidity Trends</h3>
